-- =======================================================
-- BLOX FRUITS: AUTO FARM FRUIT, GACHA, ESP, ULTRA HOP & SAVE SETTING
-- =======================================================
...
-- =======================================================
-- HỆ THỐNG LƯU & LOAD SETTING (WORKSPACE/BETA)
-- =======================================================
...
-- Khởi tạo giá trị mặc định hoặc load từ file
...
if savedConfig then
    autoFarmEnabled = savedConfig.AutoFarm or false
    ...
    writeLog("Đã tải thành công setting cũ từ file!")
end
...
local function ultraFastHop()
    ...
    local function tryTeleport(targetJob)
        ...
        if success then
            ...
            writeLog("[CẢNH BÁO] Server " .. tostring(targetJob) .. " đã đầy. Đang đổi server khác...", true)
            getgenv()._BF_IS_HOPPING = false
            task.spawn(function()
                task.wait(1)
                ultraFastHop()
        ...
        for page = 1, 15 do
            ...
            if type(pageData) == "table" and next(pageData) and not hopped then
                ...
                if #validList > 0 and not hopped then
                    ...
                    writeLog("Đã tìm thấy Server ở trang " .. page .. ", chuẩn bị Teleport sang JobId: " .. tostring(target))
                    tryTeleport(target)
                    return
                end
            end
    ...
    task.delay(12, function()
        if not hopped then
            getgenv()._BF_IS_HOPPING = false
            writeLog("Hop server quá thời gian, đang thử lại...", true)
...
local function topos(Tween_Pos, onComplete)
    ...
    -- Dưới 300 mét dịch chuyển tức thời
    ...
    currentTween.Completed:Connect(function(playbackState)
        if playbackState == Enum.PlaybackState.Completed then
            ...
            if hrp then
                ...
                local raycastResult = Workspace:Raycast(hrp.Position, Vector3.new(0, -100, 0), raycastParams)
        ...
        while autoGachaConfig do
            pcall(function()
                ...
                if commF then
                    ...
                    if type(result) == "string" and result ~= "" and result ~= "Success" then
                        sendWebhook("🎁 Gacha Trái Thành Công!", "Người chơi **" .. LocalPlayer.Name .. "** vừa quay Gacha ra trái: **" .. result .. "**", 15158332)
                    end
                    
                    task.wait(0.5)
                    autoStoreFruit(true, type(result) == "string" and result or nil)
                end
            end)
            task.wait(15)
        end
    ...
    task.spawn(function()
        ...
        while autoFarmEnabled do
            ...
            if #fruits == 0 then
                ...
                writeLog("Không có trái nào trên map. Tiến hành Hop...")
                task.wait(1)
                ultraFastHop()
                break
            else
...
-- Nếu khởi động lại game mà file cấu hình lưu trạng thái đang bật -> Tự động chạy luôn
if autoFarmEnabled then
    task.spawn(function()
        task.wait(1)
        startOneClickLoop()
